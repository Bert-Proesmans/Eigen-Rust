...
/// Comparable to anonymous functions. These instances can be invoked
/// by calling `run(..)` and will execute the embedded code.
///
/// Methods don't take arguments to run and work entirely based on
/// information within the shared state, which is held by the game
/// instance.
