...
pub trait ICardContainer
    ...
    /// WILD card set
    fn all_wild(&self) -> &Vec<&(ICard + 'static)>;

    /// Returns a vector of all cards which belong to the
    /// STANDARD card set
    fn all_standard(&self) -> &Vec<&(ICard + 'static)>;

    /// Returns a vector of all cards belonging to the WILD
    /// card set, grouped
    /// by their card class
    fn wild(&self) -> &HashMap<ECardClasses, Vec<&(ICard + 'static)>>;

    /// Returns a vector of all cards belonging to the
    /// STANDARD card set, grouped
    /// by their card class
    fn standard(&self) -> &HashMap<ECardClasses, Vec<&(ICard + 'static)>>;
