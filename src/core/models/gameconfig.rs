...
use enums::internal::errors::GameConfigError;
...
impl GameConfig {
    ...
    pub fn validate(self) -> Result<Self, GameConfigError> {
        try!(self.game_format.as_ref().ok_or(GameConfigError::NoFormat));

        if self.starting_player_idx < 1 || self.starting_player_idx > MAX_PLAYERS {
            return Err(GameConfigError::StartingPlayerOOB);
        }

        for idx in 0..MAX_PLAYERS {
            try!(self.player_heroclasses[idx as usize].ok_or(GameConfigError::NoHeroClass(idx)));
